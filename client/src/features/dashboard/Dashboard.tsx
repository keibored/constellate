...
import { createAccountRoom, createAccountRoomInvite, listAccountRooms, setAccountRoomVisibility, type AccountRoom } from '../../services/accountRooms';
...
export function Dashboard() {
  ...
  const toggleVisibility = async (room: AccountRoom) => {
    setError(null);
    try {
      const updated = await setAccountRoomVisibility(room.id, room.visibility === 'public' ? 'private' : 'public');
      setOwnedRooms(current => current.map(item => item.id === updated.id ? updated : item));
    } catch (reason) { setError(reason instanceof Error ? reason.message : 'Cannot update this room.'); }
  };
  const copyInvite = async (room: AccountRoom) => {
    setError(null);
    try {
      const token = await createAccountRoomInvite(room.id);
      await navigator.clipboard.writeText(new URL(`/r/${room.id}?invite=${encodeURIComponent(token)}`, window.location.origin).href);
    } catch (reason) { setError(reason instanceof Error ? reason.message : 'Cannot create an invitation.'); }
  };
  return <main className="product-page dashboard-page">
    ...
    {user && ownedRooms.length > 0 && <section className="owned-rooms"><p className="eyebrow">YOUR ROOMS</p>{ownedRooms.map(room => <article className="recent-room product-card" key={room.id}><div><h2>{room.name}</h2><p>r/{room.id} · {room.visibility}</p></div><div className="room-actions"><button className="secondary-button" onClick={() => void toggleVisibility(room)}>Make {room.visibility === 'public' ? 'private' : 'public'}</button>{room.visibility === 'private' && <button className="secondary-button" onClick={() => void copyInvite(room)}>Copy invite</button>}<a className="secondary-button" href={`/r/${room.id}`}>Open room</a></div></article>)}</section>}
